// XML workflow parsing and manipulation utilities
import { fixXmlTag } from "./utils";                  // XML tag fixing utility
import { DOMParser, XMLSerializer } from "@xmldom/xmldom"; // XML parsing/serialization
import {
  Workflow,                          // Workflow structure definition
  WorkflowAgent,                     // Agent configuration in workflow
  WorkflowForEachNode,               // Loop node in workflow
  WorkflowNode,                      // Base workflow node
  WorkflowTextNode,                  // Text content node
  WorkflowWatchNode,                 // Event watching node
} from "../types/core.types";
import { buildAgentTree } from "./tree"; // Agent dependency tree builder

/**
 * Parses LLM-generated XML workflow into structured workflow objects.
 *
 * This function handles the complex task of converting XML representations
 * of workflows (generated by LLMs) into the internal Workflow data structures
 * used by the execution engine. It includes robust error handling and XML
 * repair for malformed LLM outputs.
 *
 * Parsing process:
 * 1. Extract XML content between <root> tags
 * 2. Repair malformed XML if workflow is incomplete
 * 3. Parse XML into DOM and extract workflow components
 * 4. Build agent dependency relationships
 * 5. Validate workflow structure and completeness
 *
 * @param taskId - Unique identifier for the workflow/task
 * @param xml - Raw XML string generated by LLM planning
 * @param done - Whether the workflow represents a completed plan
 * @param thinking - Optional reasoning/thought process from LLM
